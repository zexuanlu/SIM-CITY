...
public class PersonAgent extends Agent implements Person{
	...
	private void goToAndDoEvent(SimEvent e){		
		...
		if(e.type == EventType.CustomerEvent){
			for(MyRole mr : roles){
				if(mr.type.equals("Rest 5 Customer")){
					((CustomerAgent5)mr.role).customerGui.setPresent(true);
					((CustomerAgent5)mr.role).gotHungry();
					mr.setActive(true);
					gui.setPresent(false);
					toDo.remove(e);
					return;
		...
		print("Customer not found");
		CustomerAgent5 cRole = new CustomerAgent5(this.name, this);
		MyRole newRole = new MyRole(cRole, "Rest 5 Customer");
		newRole.setActive(true);
		roles.add(newRole);
		CustomerGui5 cg = new CustomerGui5(cRole);
		cg.isPresent = true;
		cRole.setGui(cg);
		cap.rest5Panel.addGui(cg);
		cRole.setHost(((HostAgent5)rest.getHost()));
		cRole.setCashier(((CashierAgent5)rest.getCashier()));
		((CustomerAgent5)cRole).customerGui.setPresent(true);
		cRole.gotHungry();
		gui.setPresent(false);
		toDo.remove(e);
		}
		...
		MyRole newRole = new MyRole(((HostAgent5)rest.getHost()), "Rest 5 Host");
		newRole.setActive(true);
		roles.add(newRole);
		rest.getTimeCard().msgBackToWork(this, newRole.role);
		try{
		...
		((WaiterAgent5)mr.role).waiterGui.isPresent = true;
		rest.getTimeCard().msgBackToWork(this, mr.role); 
		try{
			wait.acquire();
		}
		...
		WaiterAgent5 wRole = new WaiterAgent5(this.name, this); 
		MyRole newRole = new MyRole(wRole, "Rest 5 Waiter");
		newRole.setActive(true);
		roles.add(newRole);
		WaiterGui5 wg = new WaiterGui5((WaiterBase5)newRole.role, 40);
		wg.isPresent = true;
		...
		return;
			...
			((SDWaiterAgent5)mr.role).waiterGui.isPresent = true;
			rest.getTimeCard().msgBackToWork(this, mr.role);
			try{
				wait.acquire();
			}
		...
		SDWaiterAgent5 sdRole = new SDWaiterAgent5(this.name, this);
		...
		mr.setActive(true);
		((CookAgent5)mr.role).cookGui.isPresent = true;
		gui.setPresent(false);
		return;
		}
		} 
		MyRole newRole = new MyRole(((CookAgent5)rest.getCook()), "Rest 5 Cook");//FIX
		newRole.setActive(true);
		roles.add(newRole);
		((CookAgent5)newRole.role).cookGui.isPresent = true;
		rest.getTimeCard().msgBackToWork(this, newRole.role);
		try{
			wait.acquire();
		}
		...
		rest.getTimeCard().msgBackToWork(this, (CashierAgent5)mr.role);
		try{
			wait.acquire();
		}
		catch(InterruptedException ie){
		...
		MyRole newRole = new MyRole(((CashierAgent5)rest.getCashier()), "Rest 5 Cashier");//FIX
		newRole.setActive(true);
		roles.add(newRole);
		rest.getTimeCard().msgBackToWork(this, newRole.role);
		try{
		...
		for(MyRole mr : roles){
			if(mr.type.equals("Rest 6 Cook")){                                                       
				...
				mr.setActive(true);
				((Restaurant6CookRole)mr.role).cookGui.isPresent = true;
				gui.setPresent(false);
				return;
			}
		} 
